...
| **GA** | `B1graphabs.pdf` / `.html`（单语，英文） | — | — | `93sscceiling` 表c/表d、`97instrbudget` 表a/表c、`A12opscan` 表a、`A4formal` 表c、`A5aggregate` 表b/表c、`A1consolidate` 表c | **现场读取，28 个数字逐个复算** ✅ **图文摘要**（不走 matplotlib，见下 G2） |
...
| ~~G2 Graphical abstract~~ | ✅ **已重做（2026-08-28）** | 由 `B1graphabs.py` 产出，1125×411 pt = 4500×1644 px @300 dpi、比例 2.74:1，满足 Elsevier「最小 531×1328 px (h×w)」；矢量 PDF，字体全为拉丁字体（分栏标签用 A./B./C. 而非 ①②③——后者 Helvetica 缺字，转 PDF 会回退到中文字体）。**七面板密集版**：①参考侧 ②误差预算 ③可达上界 ④设计规则 ⑤仪器侧 ⑥验证审计 ⑦可证伪诊断——依 ryan 2026-08-28 选定的 V4 dense 版式复刻。数值现场读 `93sscceiling` 表c 与 `A5aggregate` 表b。技术路线：Python 现场读工作簿 → 自包含 HTML（内联 SVG）→ Chrome headless 出**真矢量** PDF（实测 0 张嵌入位图、23 个嵌入字体、1,803 字符文本层；HTML 里 213 个文字 token 在 PDF 文本层 100% 可检出）+ 300dpi PNG。HTML 随产物留档（`figures/B1graphabs.html`），任何浏览器打开即可核对与改版式。**替换了原 2026-07-26 的 `fig_ga()`**——那一版是图 a(c) 与图 e(a) 的重新拼版，两个 panel 都是数据曲线，读者不读正文看不懂框架 |
